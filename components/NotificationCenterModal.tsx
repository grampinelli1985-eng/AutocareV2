                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                    {notifications.length > 0 ? (
                        notifications.map(n => (
                            <div
                                key={n.id}
                                className={`p-4 rounded-3xl bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 flex gap-4 transition-all hover:bg-white dark:hover:bg-slate-800 ${(n.type === 'theft' || n.type === 'sighting' || n.type === 'recovery') ? 'cursor-pointer hover:border-red-200 dark:hover:border-red-900/30' : ''}`}
                                onClick={() => (n.type === 'theft' || n.type === 'sighting' || n.type === 'recovery') && onAction?.(n)}
                            >
